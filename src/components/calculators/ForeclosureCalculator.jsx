    ...
    return (
        <div className="max-w-6xl mx-auto p-4 md:p-8 grid grid-cols-1 lg:grid-cols-12 gap-6 lg:gap-10">
            ...
            <div className="lg:col-span-5 space-y-6">
                <div className="bg-white p-6 md:p-8 rounded-3xl border border-white/40 shadow-xl shadow-slate-200/50 backdrop-blur-xl space-y-8 relative overflow-hidden">
                    ...
                    <div className="flex items-center gap-4">
                        ...
                        <div>
                            <h2 className="text-xl font-bold text-gray-900 tracking-tight">Loan Foreclosure</h2>
                            <p className="text-xs text-gray-500 font-medium mt-0.5">Calculate your savings on pre-closing</p>
                        </div>
                    </div>
                    ...
                    <div className="space-y-6">
                        ...
                        <div className="grid grid-cols-2 gap-5">
                            ...
                            <div className="space-y-2 group">
                                <label className="text-[11px] font-bold text-gray-500 uppercase tracking-widest group-focus-within:text-primary transition-colors">Remaining Tenure</label>
                                <div className="relative">
                                    <input
                                        type="number"
                                        value={remainingTenure}
                                        onChange={e => setRemainingTenure(Number(e.target.value))}
                                        className="w-full pl-4 pr-16 py-3 border border-slate-200 rounded-xl font-bold text-gray-900 outline-none focus:border-primary focus:ring-4 focus:ring-primary/10 shadow-sm transition-all"
                                    />
                                    <span className="absolute right-4 top-1/2 -translate-y-1/2 text-xs font-bold text-gray-400 uppercase bg-gray-50 px-2 py-1 rounded">Months</span>
                                </div>
                            </div>
                        </div>
                        ...
                        <div className="pt-2">
                            <div className="flex justify-between mb-3 px-1">
                                <label className="text-[11px] font-bold text-gray-500 uppercase tracking-widest">Foreclosure Charges</label>
                                <span className="text-xs font-bold text-primary bg-primary/10 px-2.5 py-1 rounded-lg ring-1 ring-primary/20">{foreclosureChargesPercent}%</span>
                            </div>
                            <input
                                type="range"
            ...
            <div className="lg:col-span-7 space-y-6">
                ...
                {/* VERDICT CARD */}
                <div className={`rounded-3xl p-8 text-white shadow-2xl relative overflow-hidden group transition-all duration-500 transform hover:scale-[1.01] ${calc.netSavings > 0 ? 'bg-gradient-to-br from-emerald-500 to-teal-700 shadow-emerald-500/20' : 'bg-gradient-to-br from-red-500 to-rose-700 shadow-red-500/20'}`}>
                    {/* Background Pattern */}
                ...
                <div className="bg-white rounded-3xl border border-white/60 shadow-xl shadow-slate-200/40 overflow-hidden backdrop-blur-xl">
                    <div className="grid grid-cols-1 md:grid-cols-2 divide-y md:divide-y-0 md:divide-x divide-gray-100">
                        <div className="p-8 hover:bg-gray-50/50 transition-colors">
                            <div className="flex items-center gap-2 mb-6">
                                <Minus className="w-4 h-4 text-amber-500" />
                                <div className="text-xs font-bold text-gray-400 uppercase tracking-widest">If Continued</div>
                            </div>

                            <div className="space-y-6">
                                <div>
                                    <span className="block text-xs font-semibold text-gray-400 uppercase mb-1">Remaining Interest</span>
                                    <span className="block text-2xl font-bold text-amber-600 font-mono tracking-tight">{toINR(Math.round(calc.interestIfContinued))}</span>
                                </div>
                                <div className="pt-4 border-t border-gray-100/80">
                                    <span className="block text-xs font-semibold text-gray-400 uppercase mb-1">Total Future Output</span>
                                    <span className="block text-xl font-bold text-gray-900">{toINR(Math.round(calc.totalIfContinued))}</span>
                                </div>
                            </div>
                        </div>

                        <div className="p-8 hover:bg-gray-50/50 transition-colors relative">
                            {/* subtle bg indicator */}
                            <div className="absolute top-0 right-0 w-16 h-16 bg-gradient-to-bl from-primary/5 to-transparent rounded-bl-3xl"></div>

                            <div className="flex items-center gap-2 mb-6">
                                <X className="w-4 h-4 text-primary" />
                                <div className="text-xs font-bold text-gray-400 uppercase tracking-widest">If Foreclosed</div>
                            </div>

                            <div className="space-y-6">
                                <div>
                                    <span className="block text-xs font-semibold text-gray-400 uppercase mb-1">Penalty Charges</span>
                                    <span className="block text-2xl font-bold text-rose-500 font-mono tracking-tight">{toINR(Math.round(calc.penalty))}</span>
                                </div>
                                <div className="pt-4 border-t border-gray-100/80">
                                    <span className="block text-xs font-semibold text-gray-400 uppercase mb-1">One-time Settlement</span>
                                    <span className="block text-xl font-bold text-gray-900">{toINR(Math.round(calc.foreclosureCost))}</span>
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    {/* Math Summary */}
                    <div className="bg-slate-50 p-4 text-[10px] md:text-xs font-mono text-center text-slate-500 border-t border-gray-100 flex flex-wrap justify-center gap-2">
                        <span>{toINR(Math.round(calc.totalIfContinued))} (Future)</span>
                        <span>-</span>
                        <span>{toINR(Math.round(calc.foreclosureCost))} (Now)</span>
                        <span>=</span>
                        <span className={calc.netSavings > 0 ? "text-emerald-600 font-bold" : "text-red-600 font-bold"}>{toINR(Math.round(calc.netSavings))}</span>
                    </div>
                </div>
            </div>
        </div>
